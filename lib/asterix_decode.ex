defmodule Asterix.Decode do
  ...
  def decode_record(asterix_record, category) do
    case category do
      21 ->
        decode_record(
          asterix_record,
          Asterix.Decode.Cat021.Ed0_26.full_fspec(),
          Asterix.Decode.Cat021.Ed0_26.field_decoding_functions()
        )
  ...
  defp decode_fspec(data, uap_blocks)
       when is_list(data) and
              is_list(uap_blocks) do
    List.foldl(uap_blocks, {[], data}, fn uap_block, {fspec, data} ->
  ...
  defp decode_record(data, full_fspec, user_application_profile)
       when is_list(data) and
              is_list(full_fspec) and
              is_map(user_application_profile) do
    {fspec, data} = decode_fspec(data, full_fspec)
  ...
  defp fspec_octet(octet, fspec_field_names)
